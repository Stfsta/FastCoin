---
snapshot_version: v6.3
generated_at: 2026-05-27 23:30 +08:00
language: zh
git_commit: 77c6088
git_branch: main
project_type: polyglot
previous_version: v6.2
previous_file: snapshot_archive/REPOSITORY_SNAPSHOT_v6.2_2026-05-27.md
line_budget: 300
---

## TL;DR

FastCoin 是一个纯本地、加密安全的个人快捷记账跨平台应用，使用 Tauri 2 (Rust) + React 18 (TypeScript) 构建，支持 Windows/macOS/Linux 桌面端及 Android 原生 APK（已配置 Release 签名）。支持三栏可拖拽布局（手机端 Tab 切换）、深色/浅色主题、中英文双语、自定义记账周期、可视化统计、AES-256-GCM 加密导入导出（完整/当日/当前周期）、Excel/CSV 导出、path.cfg 指针架构、Android SAF 内容模式导入导出、移动端安全区域避让、金额上限校验、记录展开详情、备注历史快选。数据存储在本地 SQLite，不依赖网络。

## 1. 项目概览

- **名称 / 版本**: `fastcoin` v0.3.1 (from `package.json` / `Cargo.toml`)
...
│   ├── capabilities/ # Tauri 插件权限声明 (含 fs:allow-write-file/read-file/remove)
...
- **运行时配置**:
  ...
  - 权限: `src-tauri/capabilities/default.json` (windows: ["main", "*"], 含 fs:allow-write-file/read-file/remove)
...
- **公开接口**: ~32 个 async 函数 (CRUD + getStats + exportData + exportDataToTemp + exportDataToContent + importPreview/Confirm + config 路径命令)
- **依赖模块**: MOD-TYPES
...
- **关键文件**: `config_cmd.rs` (set_db_path/reset_db_path Android 禁用), `export_cmd.rs` (export_data/export_data_to_content/export_data_to_temp async + spawn_blocking), `import_cmd.rs` (import_preview_from_content/import_confirm_from_content async + spawn_blocking)
...
**Android SAF 导出 (临时文件模式)**:
1. MOD-FEUI/ExportControls 调用 dialog.save() → SAF content URI
2. MOD-TAURI-IPC/exportDataToTemp → MOD-CMD/export_data_to_temp (async + spawn_blocking) → MOD-SVC 写入本地临时文件
3. 前端 readTextFile/readFile 读取临时文件 → writeTextFile/writeFile 写入 SAF URI → remove 清理临时文件
...
- **公开契约**:
  ...
  - Android 导出走临时文件模式 (export_data_to_temp + readTextFile/readFile + writeTextFile/writeFile + remove)
...
- **性能 / 安全热点**:
  ...
  - export_data_to_temp 临时文件写入 app 私有目录，finally 块清理
...
- `src/components/settings/ExportControls.tsx` → MOD-FEUI — Android SAF临时文件导出
...
- `src/lib/tauri.ts` → MOD-TAURI-IPC — 类型安全 IPC (~32 API, 含 exportDataToTemp)
...
- `src-tauri/src/commands/export_cmd.rs` → MOD-CMD — export_data + export_data_to_content + export_data_to_temp
- `src-tauri/src/services/expense_service.rs` → MOD-SVC — MAX_AMOUNT_CENTS校验
- `src-tauri/src/db/mod.rs` → MOD-DB — init_db_at(路径参数)
- `src-tauri/capabilities/default.json` → — ACL权限(含fs:allow-write-file/read-file/remove)
...
- **新增**: MOD-CMD/export_data_to_temp — 异步命令，Rust 端写入临时文件返回路径，绕过 Android IPC 大消息限制
- **新增**: MOD-TAURI-IPC/exportDataToTemp — 前端 IPC 封装
- **变更**: MOD-FEUI/ExportControls — Android 端导出从 content 模式改为临时文件模式（readTextFile/readFile + writeTextFile/writeFile + remove 清理）
- **变更**: MOD-FEUI/StatsDashboard — 来源名称添加 `title` 属性，hover/长按显示完整名称
- **变更**: MOD-FEUI/SpendingTrendChart — Y 轴手机端: left margin 从 -20 改为 0，width 从 40 改为 45；新增 formatYAxisTick 函数使用 K/M 缩写格式 (>=1K/10K/1M)
- **变更**: MOD-FEUI/SourceDistribution — 容器高度根据来源数量动态计算，cy 上移至 38%/40%，缩小内外半径，Legend 添加 verticalAlign/iconSize/wrapperStyle
- **变更**: MOD-CMD ACL — capabilities/default.json 新增 fs:allow-read-text-file、fs:allow-read-file、fs:allow-remove

## 11. 快照元数据

- Snapshot version: v6.3
- Generated at: 2026-05-27 23:30 +08:00
- Language: zh
- Git commit: 77c6088
- Git branch: main
- Project type: polyglot
- Previous version: v6.2
- Previous file: snapshot_archive/REPOSITORY_SNAPSHOT_v6.2_2026-05-27.md
