---
snapshot_version: v6.2
generated_at: 2026-05-27 22:00 +08:00
language: zh
git_commit: 7ab8070
git_branch: main
project_type: polyglot
previous_version: v6.1
previous_file: snapshot_archive/REPOSITORY_SNAPSHOT_v6.1_2026-05-26.md
line_budget: 300
---
...
- **名称 / 版本**: `fastcoin` v0.3.0 (from `package.json` / `Cargo.toml`)
...
│   ├── capabilities/ # Tauri 插件权限声明 (含 fs:allow-write-file)
...
- **运行时配置**:
  ...
  - 权限: `src-tauri/capabilities/default.json` (windows: ["main", "*"], 含 fs:allow-write-file)
...
- **关键文件**: `export_service.rs` (gather_full/date/period + export_fastcoin_to_content/csv_to_content/xlsx_to_bytes), `import_service.rs` (事务+外键排序+until_version), `stats_service.rs`, `expense_service.rs` (MAX_AMOUNT_CENTS=9_999_999_999 校验 + 正数校验)
...
3. 前端 plugin-fs.writeTextFile (fastcoin/CSV) / writeFile (XLSX, 需 fs:allow-write-file) 写入 URI
...
- `src/i18n/zh.json` → MOD-I18N — 中文翻译(含 amountTooLarge/paymentSource/expenseDate/recentNotes)
...
- `src-tauri/capabilities/default.json` → — ACL权限(含fs:allow-write-file)
...
- **新增**: MOD-STORE/noteHistoryStore — 备注历史 Zustand store，persist 到 localStorage，去重 LRU，上限 20 条
- **变更**: MOD-FEUI/ExpenseRow — 点击行展开完整信息（支付来源名称/类型、日期、完整备注），chevron 提示图标，删除按钮 stopPropagation
- **变更**: MOD-FEUI/NoteInput — 前 3 条备注始终显示，更多折叠在展开按钮后；删除键 × 始终可见
- **变更**: MOD-FEUI/ExpenseForm — 金额输入超过 MAX_AMOUNT_CENTS (9_999_999_999) 时 toast 拒绝；提交时防御性校验；备注提交后写入 noteHistoryStore；addToast/t 改用 useRef
- **变更**: MOD-SVC/expense_service — add_expense/update_expense 增加 amount 正数校验和 MAX_AMOUNT_CENTS 上限校验，返回 AppError::Validation
- **变更**: MOD-FEUI/AppShell — 手机端容器 overflow-hidden 改为 overflow-y-auto，修复统计页无法滚动
- **变更**: MOD-CMD ACL — capabilities/default.json 新增 fs:allow-write-file，修复 Android XLSX 导出 ACL 报错
- **变更**: MOD-FEUI/ImportControls — Android 平台文件选择器移除扩展名过滤器，允许选择任意文件
- **变更**: MOD-I18N — 新增 amountTooLarge/paymentSource/expenseDate/recentNotes/showRecentNotes/hideRecentNotes 翻译键
- **变更**: 版本号升级至 v0.3.0 (package.json / Cargo.toml / tauri.conf.json)

## 11. 快照元数据

- Snapshot version: v6.2
- Generated at: 2026-05-27 22:00 +08:00
- Language: zh
- Git commit: 7ab8070
- Git branch: main
- Project type: polyglot
- Previous version: v6.1
- Previous file: snapshot_archive/REPOSITORY_SNAPSHOT_v6.1_2026-05-26.md
