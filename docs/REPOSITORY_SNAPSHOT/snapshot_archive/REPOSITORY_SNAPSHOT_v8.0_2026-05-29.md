---
snapshot_version: v8.0
generated_at: 2026-05-29 16:30 +08:00
language: zh
git_commit: 554d5c3
git_branch: main
project_type: polyglot
previous_version: v7.1
previous_file: snapshot_archive/REPOSITORY_SNAPSHOT_v7.1_2026-05-28.md
line_budget: 300
---

## TL;DR

FastCoin 是一个纯本地、加密安全的个人快捷记账跨平台应用，使用 Tauri 2 (Rust) + React 18 (TypeScript) 构建，支持 Windows/macOS/Linux 桌面端及 Android 原生 APK（已配置 Release 签名）。支持三栏可拖拽布局（手机端 Tab 切换）、深色/浅色主题、中英文双语、自定义记账周期、可视化统计、AES-256-GCM 加密导入导出（完整/当日/当前周期）、Excel/CSV 导出、path.cfg 指针架构、Android SAF 内容模式导入导出、移动端安全区域避让、金额上限校验、记录展开详情、备注历史快选、最近记录关键词搜索与日期筛选。数据存储在本地 SQLite，不依赖网络。v0.4.0 修复了分类选择后显示错误图标（始终显示 🍔）及展开详情缺失分类信息的问题，新增了最近记录界面的关键词搜索框与日期筛选器。

## 1. 项目概览

- **名称 / 版本**: `fastcoin` v0.4.0 (from `package.json` / `Cargo.toml`)
...
- **关键文件**: `AppShell.tsx` (100dvh+popstate返回键+safe-area避让+overflow-y-auto), `ExpensePanel.tsx` (手机端Tab切换), `ExpenseForm.tsx` (金额上限校验MAX_AMOUNT_CENTS+备注历史记录+容器级onKeyDown Enter处理), `ExpenseList.tsx` (关键词搜索框+日期筛选器+useMemo过滤+无匹配空状态), `ExpenseRow.tsx` (点击展开支付来源/分类/日期/完整备注+分类图标按ID查找), `NoteInput.tsx` (前3条备注预览+折叠展开快选), `Modal.tsx` (手机端底部弹窗), `ExportControls.tsx` (Android内容模式导出+chunked base64解码 绕过fs插件作用域), `ImportControls.tsx` (Android SAF+无扩展名过滤), `GeneralSettings.tsx` (Android隐藏DataPathSetting), `SourceManager.tsx` (弹窗内Enter触发添加), `CategoryManager.tsx` (弹窗内Enter触发添加), `PeriodManager.tsx` (弹窗内Enter触发添加)
...
**最近记录搜索与筛选**:
9. MOD-FEUI/ExpenseList 本地状态 (searchText + filterDate) → 并行拉取 sources/categories 用于名称解析
10. useMemo 过滤: 日期精确匹配 AND 关键词大小写不敏感匹配（来源名/分类名/备注文本）→ 分组渲染
11. 无匹配时显示空状态提示
...
- **公开契约**:
  ...
  - MOD-FEUI/ExpenseList 搜索与日期筛选为客户端本地过滤（useMemo），不触发后端查询；过滤逻辑为 AND 组合；搜索匹配来源名/分类名/备注文本（大小写不敏感）
  - MOD-FEUI/ExpenseRow 分类显示通过 `api.getCategories()` 按 ID 查找，回退图标为 📂
- **性能 / 安全热点**:
...
- `src/components/expense/ExpenseList.tsx` → MOD-FEUI — 关键词搜索+日期筛选+useMemo过滤+空状态
- `src/components/expense/ExpensePanel.tsx` → MOD-FEUI — 手机端Tab切换
- `src/components/expense/ExpenseRow.tsx` → MOD-FEUI — 分类图标按ID查找+展开详情含分类
...
- **变更**: MOD-FEUI/ExpenseRow — 新增 `api.getCategories()` 获取分类列表，按 `expense.categoryId` 查找真实 Category 对象；硬编码 🍔 替换为 `category?.icon || "📂"`；展开详情新增分类行（图标+名称），位于支付来源与日期之间
- **新增**: MOD-FEUI/ExpenseList — 新增关键词搜索框（带🔍图标，大小写不敏感匹配来源名/分类名/备注文本）+ 日期筛选器（native date input + 清除按钮），客户端 `useMemo` 过滤，AND 组合逻辑，无匹配空状态提示
- **新增**: MOD-I18N — 5 个新翻译键: `searchPlaceholder`, `filterByDate`, `clearFilter`, `noMatch`, `noMatchHint` (中英文)
- **变更**: 应用版本 v0.3.3 → v0.4.0 (package.json, Cargo.toml, tauri.conf.json)
- **变更**: README.md 构建产物路径示例版本号 0.3.2 → 0.4.0

## 11. 快照元数据

- Snapshot version: v8.0
- Generated at: 2026-05-29 16:30 +08:00
- Language: zh
- Git commit: 554d5c3
- Git branch: main
- Project type: polyglot
- Previous version: v7.1
- Previous file: snapshot_archive/REPOSITORY_SNAPSHOT_v7.1_2026-05-28.md
