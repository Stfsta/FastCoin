---
snapshot_version: v7.1
generated_at: 2026-05-28 17:19 +08:00
language: zh
git_commit: aa9247f
git_branch: main
project_type: polyglot
previous_version: v7.0
previous_file: snapshot_archive/REPOSITORY_SNAPSHOT_v7.0_2026-05-28.md
line_budget: 300
---

## TL;DR

FastCoin 是一个纯本地、加密安全的个人快捷记账跨平台应用，使用 Tauri 2 (Rust) + React 18 (TypeScript) 构建，支持 Windows/macOS/Linux 桌面端及 Android 原生 APK（已配置 Release 签名）。支持三栏可拖拽布局（手机端 Tab 切换）、深色/浅色主题、中英文双语、自定义记账周期、可视化统计、AES-256-GCM 加密导入导出（完整/当日/当前周期）、Excel/CSV 导出、path.cfg 指针架构、Android SAF 内容模式导入导出、移动端安全区域避让、金额上限校验、记录展开详情、备注历史快选。数据存储在本地 SQLite，不依赖网络。v0.3.3 修复了 Enter 键全局捕获导致桌面端重复提交及设置弹窗内误触发记账的问题，优化了 Android 导出时切换页面的 UI 卡顿和导出按钮 spinner 卡死不消失的问题。

## 1. 项目概览

- **名称 / 版本**: `fastcoin` v0.3.3 (from `package.json` / `Cargo.toml`)
...
- **变更**: MOD-FEUI/ExpenseForm — 移除全局 `document` keydown Enter 监听器，替换为容器 `<div>` 级 `onKeyDown` 处理器；当焦点在 BUTTON 时交由浏览器原生 click 处理，消除桌面端双重提交问题；Enter 事件不再跨面板冒泡
- **变更**: MOD-FEUI/SourceManager — 弹窗内容 div 添加 `onKeyDown` 处理器，Enter 触发 handleAdd/handleEdit，SELECT 标签跳过
- **变更**: MOD-FEUI/CategoryManager — 弹窗内容 div 添加 `onKeyDown` 处理器，Enter 触发 handleAdd/handleEdit
- **变更**: MOD-FEUI/PeriodManager — 弹窗内容 div 添加 `onKeyDown` 处理器，Enter 触发 handleAdd
- **变更**: MOD-FEUI/ExportControls — 同步 `atob()` + 逐字节循环替换为 chunked async `base64ToBytes()` 辅助函数（64KB 分块 + `setTimeout(0)` 让出主线程），消除 Android 导出时切换页面造成的主线程卡顿
- **修复**: MOD-FEUI/ExportControls — 移除 `mountedRef` 守卫，修复 Android SAF 文件选择器导致组件短暂卸载后 `setIsExporting(false)` 被跳过、spinner 卡死不消失的 bug
- **版本**: v0.3.2 → v0.3.3

## 11. 快照元数据

- Snapshot version: v7.1
- Generated at: 2026-05-28 17:19 +08:00
- Language: zh
- Git commit: aa9247f
- Git branch: main
- Project type: polyglot
- Previous version: v7.0
- Previous file: snapshot_archive/REPOSITORY_SNAPSHOT_v7.0_2026-05-28.md
