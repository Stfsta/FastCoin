---
snapshot_version: v6.0
generated_at: 2026-05-26 19:30 +08:00
language: zh
git_commit: 09b3c99
git_branch: main
project_type: polyglot
previous_version: v5.0
previous_file: snapshot_archive/REPOSITORY_SNAPSHOT_v5.0_2026-05-26.md
line_budget: 300
---

## TL;DR

FastCoin 是一个纯本地、加密安全的个人快捷记账跨平台应用，使用 Tauri 2 (Rust) + React 18 (TypeScript) 构建，支持 Windows/macOS/Linux 桌面端及 Android 原生 APK（已配置 Release 签名）。支持三栏可拖拽布局（手机端 Tab 切换）、深色/浅色主题、中英文双语、自定义记账周期、可视化统计、AES-256-GCM 加密导入导出（完整/当日/当前周期）、Excel/CSV 导出、path.cfg 指针架构、Android SAF 内容模式导入导出。数据存储在本地 SQLite，不依赖网络。
...
- **新增**: MOD-SIGN — Android Release APK 签名配置模块
- **新增**: `src-tauri/gen/android/keystore.properties` — 签名凭证文件 (storeFile/keyAlias/passwords, git-ignored)
- **新增**: `src-tauri/gen/android/app/fastcoin-release.jks` — RSA 2048 自签名密钥库 (git-ignored)
- **变更**: `src-tauri/gen/android/app/build.gradle.kts` — 新增 `keystoreProperties` 读取 + `signingConfigs.release` + release 绑定 signingConfig
- **变更**: `.gitignore` — 新增排除 `keystore.properties` 和 `fastcoin-release.jks`
- **变更**: `README.md` — 新增「签名配置」章节，说明密钥库生成和 keystore.properties 配置步骤

## 11. 快照元数据

- Snapshot version: v6.0
- Generated at: 2026-05-26 19:30 +08:00
- Language: zh
- Git commit: 09b3c99
- Git branch: main
- Project type: polyglot
- Previous version: v5.0
- Previous file: snapshot_archive/REPOSITORY_SNAPSHOT_v5.0_2026-05-26.md
